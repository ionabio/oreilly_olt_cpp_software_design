 ...
 *
 * This file is part of the C++ training by Klaus Iglberger. The file may only
 *be used in the context of the C++ training or with explicit agreement by Klaus
 *Iglberger.
 *
 * Step 1: Refactor the given External Polymorphism solution into Type Erasure.
 *The 'Shape' class may require all types to provide a 'free_draw()' function
 *that draws them to the screen.
 *
 * Step 2: Switch from one to another graphics library. Discuss the feasibility
 *of the change: how easy is the change? How many pieces of code on which level
 *of the architecture have to be touched?
 *
 * Task 3: Add the new feature to serialize shapes by means of the
 *'FastSerialization' library. Observe how well the new code can be integrated
 *and how many new dependencies are created.
 *
 **************************************************************************************************/
...
class Circle {
public:
  explicit Circle(double radius) : radius_{radius}, center_{} {}
...
class Square {
public:
  explicit Square(double side) : side_{side}, center_{} {}
...
class Shape // Type Erasure
 {
private:
  class ShapeConcept { //External Polymorphism
  public:
  ...
  class ShapeModel : public ShapeConcept { //Strategy
  public:
    ShapeModel(ShapeT const &shape, DrawStrategy && drawer)
        : shape_{shape}, drawer_{std::move(drawer)} {}
...
public:
  ...
  Shape(ShapeT const &shape, DrawStrategy  &&drawer)
      : pimpl_{std::make_unique<ShapeModel<ShapeT, DrawStrategy>>(shape,
                                                                  std::move(drawer))} {}
...
int main() {
  ...
  shapes.emplace_back(
      Square{1.2}, GLDrawer(gl::Color::green));
